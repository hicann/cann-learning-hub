...
def grade(user_globals):
    ...
    answer_file = os.path.join(notebook_dir, 'answer', '01_answer.txt')
    if not os.path.exists(answer_file):
        answer_file = './answer/01_answer.txt'
    if not os.path.exists(answer_file):
        answer_file = os.path.join(notebook_dir, 'quick_start', 'cann_basics', 'answer', '01_answer.txt')
