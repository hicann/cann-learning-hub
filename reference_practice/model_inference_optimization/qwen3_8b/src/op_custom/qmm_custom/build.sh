...
ASCEND_CANN_PATH_OPT="-DASCEND_CANN_PACKAGE_PATH=$ASCEND_HOME_PATH"
...
then
  if [ "$cmake_version" \< "3.19.0" ] ; then
    cmake -S . -B "$BUILD_DIR" $opts "$ASCEND_CANN_PATH_OPT" -DENABLE_CROSS_COMPILE=0
  else
    cmake -S . -B "$BUILD_DIR" --preset=default "$ASCEND_CANN_PATH_OPT" -DENABLE_CROSS_COMPILE=0
  fi
  ...
  if [ "$cmake_version" \< "3.19.0" ] ; then
    cmake -S . -B "$BUILD_DIR" $opts "$ASCEND_CANN_PATH_OPT" -DHOST_NATIVE_TILING_LIB=$host_native_tiling_lib
  else
    cmake -S . -B "$BUILD_DIR" --preset=default "$ASCEND_CANN_PATH_OPT" -DHOST_NATIVE_TILING_LIB=$host_native_tiling_lib
  fi
  cmake --build "$BUILD_DIR" --target binary -j$(nproc)
  mkdir -p "$BUILD_DIR/op_kernel/binary/config"
  cmake --build "$BUILD_DIR" --target $target -j$(nproc)
else
  if [ "$cmake_version" \< "3.19.0" ] ; then
    cmake -S . -B "$BUILD_DIR" $opts "$ASCEND_CANN_PATH_OPT"
  else
      cmake -S . -B "$BUILD_DIR" --preset=default "$ASCEND_CANN_PATH_OPT"
  fi
  cmake --build "$BUILD_DIR" --target binary -j$(nproc)
  mkdir -p "$BUILD_DIR/op_kernel/binary/config"
  cmake --build "$BUILD_DIR" --target $target -j$(nproc)
