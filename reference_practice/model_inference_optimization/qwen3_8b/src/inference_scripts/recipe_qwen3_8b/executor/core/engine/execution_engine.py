...
from executor.core.kv_cache.cache_utils import allocate_cache_tensors, calculate_block_num, \
    get_kvp_local_num_tokens, prepare_block_tables, prepare_slot_mapping, validate_cache_info
...
class ExecutionEngine:
    ...
    def __init__(self, infer_config: InferenceConfig):
        ...
        self.kvp_size = self.infer_config.parallel_config.kvp_size
        self.kvp_rank = (
            self.infer_config.parallel_config.global_rank % self.kvp_size
            if self.kvp_size > 1 else 0
        )
        self.kvp_seq_partition = False
        ...
        self.ffn_world_size = self.world_size // 2 if self.enable_afd else 0
        self.is_afd_ffn_rank = self.enable_afd and self.global_rank < self.ffn_world_size
        self._init_device()
    ...
    def _init_cache_manager(self, cache_info: ModelCacheInfo):
        ...
        self.kvp_seq_partition = cache_info.kvp_seq_partition and self.kvp_size > 1
        if self.kvp_seq_partition:
            parallel_config = self.infer_config.parallel_config
            if not parallel_config.uses_kvp_cache_parallel():
                raise ValueError(
                    "KVP sequence-partition cache requires kvp_size > 1, "
                    "attn_tp_size == 1 and cp_size == 1."
                )
            if self.next_n > 0:
                raise ValueError("KVP sequence-partition cache does not support MTP next_n > 0.")
        max_table_tokens = self.max_total_len
        if self.kvp_seq_partition:
            max_table_tokens = get_kvp_local_num_tokens(self.max_total_len, self.kvp_size, self.kvp_rank)
        self.block_table_max_len = int((max_table_tokens + self.block_size - 1) / self.block_size)
        block_num_by_type = calculate_block_num(
            ...
            kvp_seq_partition=self.kvp_seq_partition,
        )
    ...
    def _build_model_inputs(
        ...
        if is_prefill:
            ...
            if self.exe_mode == "npugraph_ex":
                ...
                if self.kvp_seq_partition and self.kvp_size > 1:
                    actual_seq_lengths_kv_local = (
                ...
                actual_seq_lengths_list_kv = actual_seq_lengths_kv_local.detach().cpu().numpy().tolist()
        ...
        if self.kvcache_manager:
            ...
            slot_mapping = prepare_slot_mapping(position_ids, actual_seq_lengths_cu_q, block_tables, self.block_size,
                                                kvp_size=self.kvp_size, kvp_rank=self.kvp_rank,
                                                kvp_seq_partition=self.kvp_seq_partition)
        else:
            block_tables = None
            slot_mapping = None
    ...
    def warm_up(self):
        ...
        run_prefill_warmup = self.infer_config.disagg_config.disaggregation_mode in ["NONE", "PREFILL"]
        if self.infer_config.scheduler_config.decode_only and run_prefill_warmup:
            logger.info("Warm-up [Main]: skip model prefill step because decode_only is enabled.")
            run_prefill_warmup = False

        if run_prefill_warmup:
    ...
    def _build_afd_ffn_inputs(self, num_tokens: int, is_prefill: bool) -> Dict:
        return self.main_worker.model.build_afd_inputs(
            num_tokens, is_prefill, dtype=self.main_worker.dtype, device=self.device
        )

    def _warm_up_afd_ffn(self):
        prefill_batch_size, decode_batch_size, seq_len = self._get_warmup_shape()
        logger.info("Warm-up [AFD FFN]: executing model prefill step...")
        model_inputs = self._build_afd_ffn_inputs(prefill_batch_size * seq_len, is_prefill=True)
        self.main_worker.inference(model_inputs, is_prefill=True)

        logger.info("Warm-up [AFD FFN]: executing model decode step...")
        q_len = 1 if self.next_n == 0 else self.next_n + 1
        model_inputs = self._build_afd_ffn_inputs(decode_batch_size * q_len, is_prefill=False)
        if self.exe_mode in ["ge_graph", "npugraph_ex"]:
            logger.info("Warm-up: triggering graph compilation...")
            self.main_worker.compile_model()
        self.main_worker.inference(model_inputs, is_prefill=False)
    ...
    def forward_batch(self, batch: Batch) -> Dict[str, Any]:
        ...
        if self.is_afd_ffn_rank:
            return self.forward_afd_ffn_batch(batch)
    ...
    def forward_afd_ffn_batch(self, batch: Batch) -> Dict[str, Any]:
        self.profiler.set_status(batch.is_prefill)
        if batch.is_prefill:
            num_tokens = batch.total_tokens
        else:
            q_len = 1 if self.next_n == 0 else self.next_n + 1
            num_tokens = len(batch.requests) * q_len

        model_inputs = self._build_afd_ffn_inputs(num_tokens, batch.is_prefill)
        ...
        if batch.is_prefill:
            ...
            next_tokens = torch.zeros((actual_batch, q_len), dtype=torch.long, device=self.device)

        if self.next_n > 0:
            if batch.mtp_infos is None:
                batch.mtp_infos = MTPInfo()
            batch.mtp_infos.set_mtp_info(
                accepted_num=torch.zeros((actual_batch,), dtype=torch.long, device=self.device),
                spec_tokens=torch.zeros((actual_batch, self.next_n), dtype=torch.long, device=self.device),
            )
