...
def is_image_ext(fname):
    """
    Check if the file suffix is an image.

    Args:
        fname (str): Name.

    Returns:
        bool, if the extension is an image.
...
def open_image_folder(source_dir, *, max_images):
    ...
    Args:
        ...
        max_images (int): Maximum number of output images.
...
def open_image_zip(source, *, max_images):
    """
    Open source image zip folder and return the iteration of images.

    Args:
        source (str): Source image folder.
        max_images (int): Maximum number of output images.
    ...
    Examples:
        >>> num_files, input_iter = open_image_zip(source, max_images=max_images)
    """
    ...
    def iterate_images():
        ...
        with zipfile.ZipFile(source, mode='r') as zp:
            for idx, fname in enumerate(input_images):
                with zp.open(fname, 'r') as files:
...
def open_lmdb(lmdb_dir, *, max_images):
    """
    Open lmdb dataset (lsun_car) and return the iteration of images.

    Args:
        lmdb_dir (str): Lmdb image folder.
        max_images (int): Maximum number of output images.
    ...
    Examples:
        >>> num_files, input_iter = open_lmdb(lmdb_dir, max_images=max_images)
    """
    ...
    def iterate_images():
        ...
        with lmdb.open(lmdb_dir, readonly=True, lock=False).begin(write=False) as txn1:
            for idx, (_key, value) in enumerate(txn1.cursor()):
                try:
                    try:
...
def make_transform(transform, output_width, output_height, resize_filter):
    ...
    def center_crop_wide(width, height, img):
        """
        Wide crop transform images for lsun car 512x384.
    ...
    if transform == 'center-crop':
        if (output_width is None) or (output_height is None):
            print('Error: must specify --width and --height when using ' + transform + 'transform')
            sys.exit(1)
...
def open_dataset(source, *, max_images):
    ...
    if os.path.isdir(source):
        ...
        print(f'Error: Missing input file or directory: {source}')
        sys.exit(1)
...
def open_dest(dest):
    ...
    if dest_ext == 'zip':
        ...
        def zip_write_bytes(fname, data):
            """
            Create in memory zip and add file 'fname'.
            """
...
def convert_dataset(args_dataset):
    ...
    Args:
        args_dataset (argparse.Namespace): Arguments for converting datasets.
    ...
    PIL.Image.init()
    if dest == '':
        print('Error: Destination output filename or directory must not be an empty string')
    ...
    for idx, image in tqdm(enumerate(input_iter), total=num_files):
        ...
        if dataset_attrs is None:
            ...
            if width != height:
                print(f'Error: Image dimensions are required to be square. Got {width}x{height}')
            if dataset_attrs['channels'] not in [1, 3]:
                print('Error: Input images must be stored as RGB or grayscale')
            if width != 2 ** int(np.floor(np.log2(width))):
                print('Error: Image width/height after scale and crop are required to be power-of-two')
        elif dataset_attrs != cur_image_attrs:
            err = [f'dataset {k}/cur image {k}: {dataset_attrs[k]}/{cur_image_attrs[k]}' for k in dataset_attrs.keys()]
            print(f'Error: Image {archive_fname} attributes must be equal.  Got:\n' + '\n'.join(err))
...
def parse_args():
    ...
    args.add_argument('--source', help='Directory for input dataset', default='./ffhq.zip/', metavar='PATH')
    args.add_argument('--dest', help='Output archive name for output dataset, must end with "zip"',
                      default='./transformed_data.zip', metavar='PATH')
    args.add_argument('--max_images', help='Output only up to `max-images` images', type=int, default=None)
