...
class VcsStreamClient:
    ...
    def __init__(self, user_name=None, password=None, domain_name=None, region=None, project_id=None, callback=None,
        ...
        """
            声音复刻流式合成client初始化。 推荐使用ak、sk认证方式。
        ...
        :param callback:            回调类VcsStreamCallBack，用于监听websocket连接、响应、断开、错误等
        ...
        if not isinstance(callback, VcsStreamCallBack):
            logger.error('The parameter callback must be VcsStreamCallBack class')
            raise ClientException('The parameter callback must be VcsStreamCallBack class')
    ...
    def _connect(self, url, headers=None):
        def _on_open(ws):
            logger.info('vcs stream websocket open')
        ...
        def _on_close(ws, close_status_code='1000', close_msg='close'):
            logger.info('vcs stream websocket close')
        ...
        if not self._check_connect():
            logger.warning("status %s can't connect vcs stream,it will not execute connect" % self._status)
            return
        headers = self._get_headers(url, headers)
        retry_count = 3
    ...
    def synthesis(self, request, headers=None):
        try:
            self._connect_to_websocket(headers)
    ...
    def _connect_to_websocket(self, headers=None):
    ...
    def sendStart(self, request):
        ...
        if self._check_request(request):
            ...
            self._last_recieve_server_message_time = time.time()
            self._ws.send(message, opcode=websocket.ABNF.OPCODE_TEXT)
    ...
    def _check_request(self, request):
        if not isinstance(request, VcsStreamRequest):
            error_msg = 'The parameter of request in VcsStreamClient should be VcsStreamRequest class'
            logger.error(error_msg)
            raise ClientException(error_msg)
        if self._status == WebsocketStatus.STATE_CONNECTED:
            return True
    ...
    def _get_headers(self, url, origin_headers):
        if self._use_aksk:
        ...
        if origin_headers is not None:
            for key in origin_headers:
                headers[key] = origin_headers[key]
        return headers
    ...
    def send_msg_batch(self, path):
        try:
            ...
            raise IOError(f"Read file occurs error: {e}")
        except Exception as e:
