[![昇腾AI端云协同与多模态机械臂综合实验](./images/READMEImage.png)](https://e.huawei.com/cn/talent/learning/#/zone?customizedZoneId=vna8_362dTFq_hF6GD9wHUAfG6k)
...
本课程的数据集、模型等大文件均**托管在外部平台，运行时自动下载**，仓库内只保留代码、配置与必要的教学数据：

### 随仓库分发

| 资源 | 位置 | 说明 |
| --- | --- | --- |
| 甄嬛数据集 | `03_llm_finetuning/src/huanhuan.json` | 3729 条对话，LoRA 微调教学数据 |
| 华为云 SIS SDK | `02_speech_ocr/src/huaweicloud_sis/` | 35 个文件，02 章运行必需（未发布至 PyPI） |
| YOLO 训练配置 | `01_yolo_detection/src/fruit.yaml` | 数据集配置（数据本身运行时下载） |

### 运行时自动下载（notebook / README 内置下载逻辑）

| 资源 | 来源 | 使用位置 | 大小 |
| --- | --- | --- | --- |
| YOLOv10n 权重 | Ultralytics 官方（首次运行 `YOLO()` 自动下载） | 01.03 / 01.04 | 5.6MB |
| 水果检测数据集（93 张，5 类） | ModelScope `Kumako/yolo_fruit_dataset` | 01.04 自动下载 | 155MB |
| 声音复刻参考音频 | ModelScope `Kumako/speech_audio_samples` | 02.04 自动下载 | 1.4MB |
| DeepSeek-R1-1.5B 模型 | ModelScope `deepseek-ai/DeepSeek-R1-Distill-Qwen-1.5B` | 03.02 自动下载 | 3.3GB |
...
| 香橙派分步教学脚本 | ModelScope `Kumako/orange_pi_teaching_scripts` | 第 5 章 README 按需下载 | 34MB |

> 💡 大文件已托管在 [魔搭 ModelScope](https://modelscope.cn)，运行 notebook 时自动检测本地缓存、按需下载（国内高速，无需登录）。
