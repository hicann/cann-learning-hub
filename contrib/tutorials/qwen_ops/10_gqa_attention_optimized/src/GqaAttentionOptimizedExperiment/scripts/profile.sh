...
BINARY="$ROOT_DIR/out/bin/gqa_attention_optimized_standalone"
[[ -x "$BINARY" ]] || { echo "[ERROR] Run scripts/build.sh first."; exit 1; }
if [[ -n "${ASCEND_HOME_PATH:-}" ]]; then CANN_PATH="$ASCEND_HOME_PATH"; elif [[ -d /usr/local/Ascend/ascend-toolkit/latest ]]; then CANN_PATH=/usr/local/Ascend/ascend-toolkit/latest; else echo "[ERROR] Set ASCEND_HOME_PATH"; exit 1; fi
