...
MSPROF="${MSPROF:-}"
if [[ -z "$MSPROF" ]]; then
    MSPROF=$(command -v msprof || true)
fi
if [[ -z "$MSPROF" ]]; then
    for p in "$HOME"/Ascend/ascend-toolkit/*/tools/profiler/bin/msprof \
             /usr/local/Ascend/ascend-toolkit/*/tools/profiler/bin/msprof; do
        if [[ -x "$p" ]]; then
            MSPROF="$p"
            break
        fi
...
if [[ -z "$MSPROF" || ! -x "$MSPROF" ]]; then
    echo "[ERROR] msprof not found. Set MSPROF=/path/to/msprof"
    exit 1
fi
...
# ── 运行 msprof (wrapper 方式) ───────────────────────────
APP_WRAPPER="$ROOT_DIR/output/profile/run_rope_opt_profile.sh"
...
WRAPPER_EOF
chmod +x "$APP_WRAPPER"
echo "[PROFILE] Running msprof via wrapper..."
...
echo "[PROFILE] Done. Output: output/profile/"
echo "[PROFILE] View with: msprof --parse=on --output=output/profile"
