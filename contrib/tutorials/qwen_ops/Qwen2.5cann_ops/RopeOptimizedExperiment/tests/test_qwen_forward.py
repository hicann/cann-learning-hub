...
def patch_qwen_rope(model):
    ...
    require_npu_runtime()
    load_torch_ops()
    rope_qk_npu = torch.ops.qwen_rope_custom_opt.rope_qk_compact

    # warmup dispatcher
    d = torch.randn(2, 64)
    rope_qk_npu(d, d, d, d, 2, 1, 1)
    ...
    for layer in model.model.layers:
        ...
        def custom_forward(self, hidden_states, *args,
                           ...
                           **kwargs):
            # Compatible with both Qwen2 APIs:
            #   old: forward(hidden_states, attention_mask, position_ids, ...)
            #   new: forward(hidden_states, position_embeddings, attention_mask, ...)
            if args:
                first = args[0]
            ...
            # Q/K RoPE: compact cos/sin are shared by all heads and both launches.
            q_f = query_states.reshape(-1, head_dim).contiguous()
            k_f = key_states.reshape(-1, head_dim).contiguous()
            q_rope, k_rope = rope_qk_npu(q_f, k_f, cos_f, sin_f, seq_len, _num_heads, _num_kv_heads)
            query_states = q_rope.view_as(query_states)
            key_states = k_rope.view_as(key_states)
    ...
    print(f"[PATCH] {patched} attention layers → fused Q/K NPU RoPE kernel")
