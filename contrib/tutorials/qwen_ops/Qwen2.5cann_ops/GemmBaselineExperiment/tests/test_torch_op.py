...
def run_case(m: int, k: int, n: int, atol: float, rtol: float) -> bool:
    ...
    actual = torch.ops.gemm_custom.gemm(a, b)
    torch.npu.synchronize()
    diff = (actual - golden).abs()
    rel = diff / (golden.abs() + 1e-12)
    print(f"M={m} K={k} N={n} max_abs={diff.max().item():.8e} mean_abs={diff.mean().item():.8e} max_rel={rel.max().item():.8e}")
...
def main():
    ...
    load_torch_ops()
    cases = [(1, 128, 64), (16, 256, 128), (args.m, args.k, args.n)]
    ok = all(run_case(m, k, n, args.atol, args.rtol) for m, k, n in cases)
