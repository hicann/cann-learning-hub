...
#include "acl/acl.h"
#include "aclrtlaunch_gemm_baseline_kernel.h"
...
int main(int argc, char **argv)
{
    ...
    if (aElems > UINT32_MAX || bElems > UINT32_MAX || cElems > UINT32_MAX) {
        std::fprintf(stderr, "[ERROR] matrix is too large for this baseline demo\n");
        return -1;
    }

    const uint32_t blockDim = std::max(1u, std::min(opt.blockDim, opt.m));
    GemmBaselineTiling tiling;
    tiling.m = opt.m;
    tiling.n = opt.n;
    tiling.k = opt.k;
    tiling.coreNum = blockDim;
    tiling.rowsPerCore = (opt.m + blockDim - 1u) / blockDim;
    ...
    std::printf("[INFO] GEMM baseline M=%u K=%u N=%u blockDim=%u rowsPerCore=%u\n",
        opt.m, opt.k, opt.n, blockDim, tiling.rowsPerCore);

    std::vector<float> a(aElems), b(bElems), golden(cElems), output(cElems);
    std::mt19937 rng(42);
    ...
    CHECK_ACL(aclrtMalloc(&tilingD, sizeof(GemmBaselineTiling), ACL_MEM_MALLOC_NORMAL_ONLY));

    CHECK_ACL(aclrtMemcpy(aD, aBytes, a.data(), aBytes, ACL_MEMCPY_HOST_TO_DEVICE));
    CHECK_ACL(aclrtMemcpy(bD, bBytes, b.data(), bBytes, ACL_MEMCPY_HOST_TO_DEVICE));
    ...
    for (uint32_t i = 0; i < opt.warmup; ++i) {
        ACLRT_LAUNCH_KERNEL(gemm_baseline_kernel)(blockDim, stream, aD, bD, cD, workspaceD, tilingD);
    }
