...
#include "acl/acl.h"
#include "aclrtlaunch_rmsnorm_baseline_kernel.h"
...
int main(int argc, char **argv)
{
    ...
    std::printf("[INFO] RMSNorm baseline rows=%u hidden=%u total=%u blockDim=%u rowsPerCore=%u "
                "eps=%.8g invHidden=%.8g\n",
    ...
    CHECK_ACL(aclrtMalloc(&tilingD, sizeof(RmsNormBaselineTiling), ACL_MEM_MALLOC_NORMAL_ONLY));
    ...
    for (uint32_t i = 0; i < opt.warmup; ++i) {
        ACLRT_LAUNCH_KERNEL(rmsnorm_baseline_kernel)(blockDim, stream, inputD, weightD, outD, workspaceD, tilingD);
    }
