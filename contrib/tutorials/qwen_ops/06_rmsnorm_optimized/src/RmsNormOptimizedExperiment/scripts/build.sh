...
detect_soc() {
    ...
    [[ -z "$raw" ]] && return
    local normalized
    normalized=$(echo "$raw" | sed -E 's/^[Aa][Ss][Cc][Ee][Nn][Dd]//; s/-[0-9]+$//' | tr '[:upper:]' '[:lower:]')
    if [[ "$normalized" =~ ^kirin ]]; then echo "$normalized"; else echo "ascend$normalized"; fi
}

if [[ -z "$SOC_VERSION" ]]; then
    SOC_VERSION=$(detect_soc || true)
    [[ -z "$SOC_VERSION" ]] && SOC_VERSION=ascend910b4
fi
...
python3 "$ROOT_DIR/../../../scripts/retarget_kernel_library.py" \
    ...
    --register librmsnorm_torch_register.so \
    --kernel librmsnorm_kernels_npu.so

echo "[OK] build finished"
echo "[OK] out/bin/rmsnorm_optimized_standalone"
