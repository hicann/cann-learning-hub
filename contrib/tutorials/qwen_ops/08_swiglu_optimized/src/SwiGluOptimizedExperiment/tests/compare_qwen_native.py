...
def patch_qwen_mlp_swiglu(model):
    load_torch_ops()
    ...
    if patched == 0:
        raise RuntimeError("No Qwen MLP modules were patched")

    print(f"[PATCH] patched {patched} Qwen MLP modules with custom optimized SwiGLU")
...
def main():
    ...
    parser.add_argument("--model", default=os.environ.get(
        "QWEN_OPS_MODEL_PATH",
        str(Path(__file__).resolve().parents[4] / "Models" / "Qwen2.5-0.5B"),
    ))
    ...
    print("[RUN] Qwen with custom optimized SwiGLU")
    patch_qwen_mlp_swiglu(model)
