...
DEFAULT_MODEL = Path(
    os.environ.get("QWEN_OPS_MODEL_PATH", ROOT / "Models" / "Qwen2.5-0.5B")
)
...
def operator_projects(variant):
    """Return the five source projects for one course variant."""
    if variant == "baseline":
        relative_paths = {
            "rope": "02_rope_baseline/src/RopeBaselineExperiment",
            "gemm": "04_gemm_baseline/src/GemmBaselineExperiment",
            "rms": "01_rmsnorm_baseline/src/RmsNormBaselineExperiment",
            "swiglu": "03_swiglu_baseline/src/SwiGluBaselineExperiment",
            "gqa": "05_gqa_attention_baseline/src/GqaAttentionBaselineExperiment",
        }
    elif variant == "optimized":
        relative_paths = {
            "rope": "07_rope_optimized/src/RopeOptimizedExperiment",
            "gemm": "09_gemm_optimized/src/GemmOptimizedExperiment",
            "rms": "06_rmsnorm_optimized/src/RmsNormOptimizedExperiment",
            "swiglu": "08_swiglu_optimized/src/SwiGluOptimizedExperiment",
            "gqa": "10_gqa_attention_optimized/src/GqaAttentionOptimizedExperiment",
        }
    else:
        raise ValueError(f"unsupported operator variant: {variant}")
    return {name: ROOT / relative for name, relative in relative_paths.items()}


def integration_result_path(variant):
    """Return the default result file inside the matching integration chapter."""
    relative_paths = {
        "baseline": (
            "11_baseline_ops_integration/src/"
            "Qwen2.5BaselineIntegrationExperiment/results/latest.json"
        ),
        "optimized": (
            "12_optimized_ops_integration/src/"
            "Qwen2.5OptimizedIntegrationExperiment/results/latest.json"
        ),
    }
    try:
        return ROOT / relative_paths[variant]
    except KeyError as exc:
        raise ValueError(f"unsupported operator variant: {variant}") from exc
...
def load_ops(torch, variant):
    ...
    projects = operator_projects(variant)
...
def _attention_result_arity(decoder_layer):
    """Read how many values this transformers decoder consumes from self_attn."""
    source = textwrap.dedent(inspect.getsource(type(decoder_layer).forward))
    tree = ast.parse(source)
    for node in ast.walk(tree):
        if not isinstance(node, ast.Assign) or not isinstance(node.value, ast.Call):
            continue
        callee = node.value.func
        if not isinstance(callee, ast.Attribute) or callee.attr != "self_attn":
            continue
        ...
        if arity in (2, 3):
            return arity
    raise RuntimeError(
        f"cannot determine the self_attn return contract for {type(decoder_layer).__name__}"
    )
...
def patch_attention(model, ops, variant):
    ...
    for layer in model.model.layers:
        ...
        return_arity = _attention_result_arity(layer)
        ...
        def custom_forward(
            ...
            **kwargs,
            ...
            projected = self.o_proj(result.to(dtype=hidden_states.dtype))
            if _return_arity == 3:
...
def main(default_variant):
    ...
    result = args.result or integration_result_path(default_variant)
