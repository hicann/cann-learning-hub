...
#include "acl/acl.h"
#include "aclrtlaunch_swiglu_baseline_kernel.h"
...
int main(int argc, char **argv)
{
    ...
    if (!ParseOptions(argc, argv, &opt)) {
        std::printf("Usage: %s --rows N --hidden N --block-dim N --warmup N --repeat N --rounds N\n", argv[0]);
        return argc > 1 ? -1 : 0;
    }
    ...
    const uint32_t elementsPerCore = (totalSize + blockDim - 1u) / blockDim;
    ...
    std::printf("[INFO] SwiGLU baseline rows=%u hidden=%u total=%u blockDim=%u elementsPerCore=%u\n",
        opt.rows, opt.hidden, totalSize, blockDim, elementsPerCore);
    ...
    CHECK_ACL(aclrtMalloc(&tilingD, sizeof(SwiGluTiling), ACL_MEM_MALLOC_NORMAL_ONLY));
    ...
    for (uint32_t i = 0; i < opt.warmup; ++i) {
        ACLRT_LAUNCH_KERNEL(swiglu_baseline_kernel)(blockDim, stream, gateD, upD, outD, workspaceD, tilingD);
    }
