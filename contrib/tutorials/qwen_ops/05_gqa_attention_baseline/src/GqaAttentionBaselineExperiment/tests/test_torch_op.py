...
def reference(q, k, v, causal):
    ...
    scores = torch.matmul(q, expanded_k.transpose(-1, -2)) / math.sqrt(q.size(-1))
    if causal:
        q_pos = torch.arange(q.size(-2)).view(-1, 1)
        k_pos = torch.arange(k.size(-2)).view(1, -1)
        scores = scores.masked_fill(k_pos > q_pos + k.size(-2) - q.size(-2), float("-inf"))
    return torch.softmax(scores, dim=-1) @ expanded_v


def run_case(batch, q_heads, kv_heads, q_len, kv_len, head_dim, causal):
    torch.manual_seed(42)
    q = torch.randn(batch, q_heads, q_len, head_dim, dtype=torch.float32)
    k = torch.randn(batch, kv_heads, kv_len, head_dim, dtype=torch.float32)
    v = torch.randn_like(k)
    golden = reference(q, k, v, causal)
    actual = torch.ops.gqa_attention_custom.gqa_attention(q, k, v, 0.0, causal)
