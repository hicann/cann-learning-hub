...
#define CHECK_ACL(call)                                                      \
    do {                                                                      \
        ...
        if (_e != ACL_SUCCESS) {                                              \
            ...
            throw std::runtime_error(std::string("ACL error ") +              \
                std::to_string(_e) + " at " + __FILE__ + ":" +                \
                std::to_string(__LINE__) +                                    \
                (_msg ? std::string(" recent_msg=") + _msg : std::string("")));\
        }                                                                     \
    } while (0)
...
inline aclTensor *CreateTensor(const std::vector<int64_t> &shape,
    ...
    if (!tensor) throw std::runtime_error("aclCreateTensor failed");
    return tensor;
}
