...
5. 完整流水线中的 TopK 性能循环会将 ACL Executor 标记为可重复执行；其余算子执行完成后立即释放 Executor。

## 复验要求

在普通开发账号下完成构建与运行，不应修改系统 CANN 的 `opp/vendors`。连续执行三次完整流程，`main_benchmark` 的三种路由索引一致，且 `verify_full_pipeline.py` 对 topk、quicksort、heapsort 均输出 `PASS`。
