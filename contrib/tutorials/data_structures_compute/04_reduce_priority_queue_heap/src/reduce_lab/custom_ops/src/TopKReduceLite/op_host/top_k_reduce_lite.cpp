...
static constexpr uint32_t VALUE_STRIDE = CACHE_LINE_BYTES / sizeof(uint16_t);
static constexpr uint32_t INDEX_STRIDE = CACHE_LINE_BYTES / sizeof(uint32_t);
static_assert(TOP_K <= VALUE_STRIDE && TOP_K <= INDEX_STRIDE, "TopK output must fit in one cache-line slot");
...
static ge::graphStatus TilingFunc(gert::TilingContext *context)
{
    ...
    tiling.set_valueStride(VALUE_STRIDE);
    tiling.set_indexStride(INDEX_STRIDE);
...
static ge::graphStatus InferShape(gert::InferShapeContext *context)
{
    ...
    *valuesShape = gert::Shape({static_cast<int64_t>(optiling::BLOCK_DIM) * optiling::VALUE_STRIDE});
    *indicesShape = gert::Shape({static_cast<int64_t>(optiling::BLOCK_DIM) * optiling::INDEX_STRIDE});
    return ge::GRAPH_SUCCESS;
}
}
