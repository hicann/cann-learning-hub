...
// GlobalTensor::SetValue writes through the per-core DCache. Give each core a
// separate 64-byte line so partial results cannot overwrite one another.
static constexpr uint32_t CACHE_LINE_BYTES = 64;
static constexpr uint32_t OUTPUT_STRIDE = CACHE_LINE_BYTES / sizeof(uint16_t);
...
static ge::graphStatus TilingFunc(gert::TilingContext *context)
{
    ...
    tiling.set_outputSize(blockDim * OUTPUT_STRIDE);
    tiling.set_outputStride(OUTPUT_STRIDE);
 ...
 * 输出 y 的 shape 为 [BLOCK_DIM * OUTPUT_STRIDE]。每个核只使用一个槽位，
 * 槽位之间隔开一个 DCache 缓存行，Host 再汇总这些 partial sum。
 * Host 端读取 y[0..7] 并求和得到最终结果
 */
static ge::graphStatus InferShape(gert::InferShapeContext *context)
{
    gert::Shape *yShape = context->GetOutputShape(0);
    *yShape = gert::Shape({static_cast<int64_t>(optiling::BLOCK_DIM) * optiling::OUTPUT_STRIDE});
    return ge::GRAPH_SUCCESS;
}
}
