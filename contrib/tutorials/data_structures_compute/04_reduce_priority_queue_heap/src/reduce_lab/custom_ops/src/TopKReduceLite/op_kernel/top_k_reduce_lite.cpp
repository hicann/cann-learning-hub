...
public:
    ...
    __aicore__ inline void Init(GM_ADDR x, GM_ADDR values, GM_ADDR indices,
                                ...
                                uint32_t blockDim, uint32_t valueStride,
        ...
        valGm.SetGlobalBuffer((__gm__ half *)values, blockDim * valueStride);
        idxGm.SetGlobalBuffer((__gm__ int32_t *)indices, blockDim * indexStride);
    }
    ...
    __aicore__ inline void Process() {
        ...
        for (uint32_t k = 0; k < topK; ++k) {
            valGm.SetValue(blockId * valueStride + k, (k < hN) ? static_cast<half>(hVal[k]) : static_cast<half>(-1e30f));
            idxGm.SetValue(blockId * indexStride + k, (k < hN) ? hIdx[k] : -1);
        }
    }
private:
    ...
    uint32_t totalLength, topK, blockLength, tileLength, tileNum, lastTileLength, blockDim, valueStride, indexStride;
};
...
__aicore__ inline void RunTopKReduceLite(GM_ADDR x, GM_ADDR values, GM_ADDR indices, GM_ADDR workspace, GM_ADDR tiling) {
    ...
    op.Init(x, values, indices, workspace, tilingData.totalLength, tilingData.topK,
            ...
            tilingData.blockDim, tilingData.valueStride, tilingData.indexStride);
    op.Process();
}
