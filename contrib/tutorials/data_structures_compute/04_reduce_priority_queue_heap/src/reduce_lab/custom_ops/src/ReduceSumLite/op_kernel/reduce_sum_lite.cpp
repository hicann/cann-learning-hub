...
public:
    ...
    __aicore__ inline void Process() {
        ...
        // Each core uses a separate 64-byte DCache slot in the output tensor.
        yGm.SetValue(blockId * outputStride, static_cast<half>(localSum));
    }

private:
    GlobalTensor<half> xGm;  // 输入 tensor (GM)
    GlobalTensor<half> yGm;  // 输出 tensor (GM)
    uint32_t totalLength, blockLength, tileLength, tileNum, lastTileLength, outputSize, outputStride;
};
...
__aicore__ inline void RunReduceSumLite(GM_ADDR x, GM_ADDR y, GM_ADDR workspace, GM_ADDR tiling) {
    ...
    op.Init(x, y, workspace, tilingData.totalLength, tilingData.blockLength,
            ...
            tilingData.outputSize, tilingData.outputStride);
    op.Process();
}
