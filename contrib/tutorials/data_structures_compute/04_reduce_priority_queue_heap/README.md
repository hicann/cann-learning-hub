...
# 算子包部署在本实验的独立 OPP 路径，不写入全局 CANN 目录
...
Host 侧的 `BLOCK_DIM` 常量会根据 `TARGET` 环境变量被 `build_ops.sh` 自动 patch（310B=8, 910B=20），并将相同值传给 runner 编译。每个 AI Core 的部分结果使用独占的 64 字节输出槽位，Host 读取各槽位首元素完成最终规约，避免多核 DCache 写回竞争。
