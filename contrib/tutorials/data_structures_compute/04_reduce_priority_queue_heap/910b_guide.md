...
# msopgen 需要读取 CANN vendor registry；无权限时请联系环境管理员，
# 不要对共享 CANN 目录执行 sudo chown/chmod。
test -r "${ASCEND_HOME_PATH}/opp/vendors/config.ini" || {
  echo "Cannot read ${ASCEND_HOME_PATH}/opp/vendors/config.ini" >&2
  exit 1
}
...
2. **缓存行隔离的部分结果**：每个 AI Core 的 Reduce/TopK 部分结果都写入独占的 64 字节槽位；Host 再读取每个槽位的首元素做最终合并，避免多核 DCache 写回竞争
...
首次通过后，请保持同一份输入数据，连续运行 50 次；每次三个算子都必须为 `PASS`。

```bash
mkdir -p data/output
for i in $(seq 1 50); do
  aclnn_runner/build/main_reduce_benchmark data 1024 4 | tee "data/output/reduce_run_${i}.log"
done
! grep -R "FAIL" data/output/reduce_run_*.log
```
