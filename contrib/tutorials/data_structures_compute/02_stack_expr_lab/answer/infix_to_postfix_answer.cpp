/*
 * InfixToPostfixLite — 中缀表达式转后缀表达式算子（参考答案）
 *
 * 核心思想：运算符栈辅助完成格式转换
 *   - 操作数 → 直接输出
 *   - '(' → 入栈
 *   - ')' → 弹出栈顶运算符到输出，直至遇到 '('
 *   - 运算符：优先级 > 栈顶 → 入栈；否则弹出栈顶到输出
 */
...
private:
    ...
    char ubOptrStack[MAX_OPTR_STACK];  // Local Memory 上的运算符栈
    char ubOutput[MAX_OUTPUT];         // Local Memory 上的输出缓冲区
};
