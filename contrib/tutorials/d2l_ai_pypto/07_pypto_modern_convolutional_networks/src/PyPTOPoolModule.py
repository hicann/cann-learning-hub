...
def _max_pool_forward(X, kH, kW, s_h, s_w, pad):
    ...
    idx = cols.argmax(dim=1)                     # bwd 复用, 免重算 im2col+argmax
    return Y, idx


# 通用路径 bwd 的每窗口位置偏移基数缓存 (只依赖形状, 跨 step 复用)
_MAXPOOL_BASE_CACHE = {}


def _maxpool_base_offsets(NC, Ho, Wo, Hp, Wp, s_h, s_w, dev):
    key = (NC, Ho, Wo, Hp, Wp, s_h, s_w)
    base = _MAXPOOL_BASE_CACHE.get(key)
    if base is None:
        m = torch.arange(NC * Ho * Wo, device=dev)
        base = (m // (Ho * Wo)) * (Hp * Wp) + \
            ((m % (Ho * Wo)) // Wo) * (s_h * Wp) + (m % Wo) * s_w
        _MAXPOOL_BASE_CACHE[key] = base
    return base


def _max_pool_backward(grad_Y, X, kH, kW, s_h, s_w, pad, idx):
    """host 实现: 稀疏 col2im 散射 (仅 argmax 位置 index_add)。

    对比旧版 (one-hot 掩码 + 全量 col2im 散射, bwd 实测 2.3ms):
    1. fwd 已保存 argmax 索引, bwd 免重算 im2col (0.62ms) + argmax (0.22ms);
    2. 每窗口 9 个候选缩减为 1 个散射点, one_hot (0.29ms) + 全量 col2im
       (1.34ms) 替换为整数算术索引 + 稀疏 index_add (0.24ms)。
    (4,64,48,48) k3s2p1 上 bwd 实测 ~0.55ms。
    """
    N, C, H, W = X.shape
    H_out, W_out = grad_Y.shape[2], grad_Y.shape[3]
    Hp, Wp = H + 2 * pad, W + 2 * pad
    dev = grad_Y.device
    if idx is None:                              # 快路径 (stride==kernel): 未存索引则重算
        X_pad = F.pad(X.contiguous(), [pad, pad, pad, pad], value=float("-inf"))
        cols = _im2col(X_pad, kH, kW, s_h, s_w)
        idx = cols.argmax(dim=1)
    base = _maxpool_base_offsets(N * C, H_out, W_out, Hp, Wp, s_h, s_w, dev)
    # argmax 索引 → (kh, kw) 窗口内偏移: kh = idx//kW, kw = idx%kW (idx < kH*kW)
    sel = (idx // kW) * Wp + (idx % kW) + base
    out = torch.zeros(N * C * Hp * Wp, device=dev, dtype=grad_Y.dtype)
    out.index_add_(0, sel, grad_Y.contiguous().reshape(-1))
    grad_X = out.reshape(N, C, Hp, Wp)
    if pad > 0:
        grad_X = grad_X[:, :, pad:pad + H, pad:pad + W]
    return grad_X
...
class PyPTOMaxPool2dFunction(torch.autograd.Function):
    @staticmethod
    def forward(ctx, X, kH, kW, s_h, s_w, pad):
        Y, idx = _max_pool_forward(X, kH, kW, s_h, s_w, pad)
        ctx.params = (kH, kW, s_h, s_w, pad)
        if idx is not None:
            ctx.save_for_backward(X, idx)       # 通用路径: bwd 复用 argmax 索引
        else:
    ...
    def backward(ctx, grad_output):
        ...
        idx = ctx.saved_tensors[1] if len(ctx.saved_tensors) > 1 else None
        return (_max_pool_backward(grad_output, X, *ctx.params, idx),
                None, None, None, None, None)
