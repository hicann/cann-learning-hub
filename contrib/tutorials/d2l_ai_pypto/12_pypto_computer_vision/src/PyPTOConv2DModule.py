...
# ── M 维分块: matmul codegen 会把整个 tile 调度静态展开成直线代码,
#    体积 ∝ tile 总数(≈M·N/tile面积), 大 shape 单 kernel 可达 MB 级/编译数十分钟.
#    限制单 kernel tile 数, 把大 matmul 切成若干同 shape 小 kernel (编译一次, 复用所有层).
_MM_MAX_TILES = 64
_MM_TILE_M, _MM_TILE_N = 128, 128


def _mm_rows_cap(M, N):
    """单次 matmul 的 M 上限, 使 tile 数 ≤ _MM_MAX_TILES."""
    n_tn = max(1, -(-int(N) // _MM_TILE_N))
    rows = (_MM_MAX_TILES // n_tn) * _MM_TILE_M
    return max(_MM_TILE_M, min(int(M), max(rows, _MM_TILE_M)))


def _matmul_bt_capped(A, B, out):
    """C = A @ B^T, M 维分块 (输出行切片, 各块独立写)."""
    M, N = A.shape[0], B.shape[0]
    step = _mm_rows_cap(M, N)
    if step >= M:
        _matmul_bt_kernel(A, B, out)
        return
    for m0 in range(0, M, step):
        m1 = min(m0 + step, M)
        _matmul_bt_kernel(A[m0:m1], B, out[m0:m1])


def _matmul_at_capped(A, B, out):
    """C = A^T @ B, M(归约)维分块 + 累加 (首块覆盖写, 后续 += )."""
    M = A.shape[0]
    step = _mm_rows_cap(M, max(out.shape))
    if step >= M:
        _matmul_at_kernel(A, B, out)
        return
    for idx, m0 in enumerate(range(0, M, step)):
        m1 = min(m0 + step, M)
        part = torch.empty_like(out)
        _matmul_at_kernel(A[m0:m1], B[m0:m1], part)
        if idx == 0:
            out.copy_(part)
        else:
            out += part
...
def _conv_forward_via_matmul(X, K, bias, stride, padding):
    ...
    out = torch.empty(N * H_out * W_out, Cout, device=X.device, dtype=X.dtype)
    _matmul_bt_capped(cols, K_2d, out)                     # C = cols @ K_2d^T (全量覆盖写, 免 fill)
    if bias is not None:
...
def _conv_dK_via_matmul(X, grad_Y, K_4d, stride, padding):
    ...
    dK_unfold = torch.empty(Cin * kH * kW, Cout, device=X.device, dtype=X.dtype)
    _matmul_at_capped(cols, dY_2d, dK_unfold)              # C = cols^T @ dY_2d
...
def _conv2d_compute_dX(grad_Y, K_4d, X_shape, stride, padding):
    ...
    out = torch.empty(cols.shape[0], Cin, device=grad_Y.device, dtype=grad_Y.dtype)
    _matmul_bt_capped(cols, K_ic, out)                     # C = cols @ K_ic^T
...
class PyPTOConv2dFunction(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        ...
        # 按需计算: 输入不需要梯度时跳过对应重路径 (如 NST 冻结骨干权重)
        ...
        db = (grad_Y.sum(dim=(0, 2, 3)).contiguous()
              if bias is not None and ctx.needs_input_grad[5] else None)
