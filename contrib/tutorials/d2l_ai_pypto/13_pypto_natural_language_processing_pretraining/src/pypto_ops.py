...
| cosine_sim | matmul + mul/rsqrt 融合余弦相似度（13.7 节 knn） | 推理场景，无反向 | cube+vec (128,8) |
...
- PyPTOCosineSim（13.7 节 knn 余弦相似度，推理）
- PyPTOFusedAttention
...
# ---------------------------------------------------------------------------
# matmul 大 M 分块：pypto 每次 kernel launch 的 workspace ≈ 动态 assemble
# outcast 张量大小 × stitch_function_max_num（实测 (8192,128,2048) 前向在
# 默认 stitch=128 时请求 8.00GiB = 8192×2048×4B×128），且由编译期
# stitch_function_max_num 与 launch shape 共同决定、运行时以单次连续分配
# torch.empty(ws, int8, npu) 申请。batch=512（M=32768）时 MLM 头前/反向
# workspace 达数百 GiB（DevCommonErr::ALLOC_FAILED）。因此：
#   1) 上述三个 kernel 的 runtime_options 均设置 stitch_function_max_num=8
#      （workspace 降 16 倍，保留少量 stitch 深度）；
#   2) 前/反向沿 M 分块（每块全新张量，不用大张量切片），使每次 launch 的
#      shape 与 batch=128 相当，同时规避大 shape 的编译时间与 workspace。
# 分块求和与整体 matmul 数学等价（仅浮点累加顺序不同）。
# ---------------------------------------------------------------------------
MATMUL_CHUNK_ROWS = 8192  # 与 batch=128 × max_len=64 的行数一致


def chunked_matmul_fwd(a, b, out, fwd_kernel):
    """matmul 前向：M 超限时按行分块，每块用全新张量，结果拷回 out。"""
    m = a.shape[0]
    if m <= MATMUL_CHUNK_ROWS:
        fwd_kernel(a.contiguous(), b.contiguous(), out)
        return
    b_c = b.contiguous()
    for s in range(0, m, MATMUL_CHUNK_ROWS):
        e = min(s + MATMUL_CHUNK_ROWS, m)
        a_c = a[s:e].clone()
        y_c = torch.empty(e - s, b.shape[1], device=a.device, dtype=a.dtype)
        fwd_kernel(a_c, b_c, y_c)
        out[s:e].copy_(y_c)


def chunked_matmul_bwd(a, b, grad_c, grad_a, grad_b, bwd_kernel):
    """matmul 反向：按 M 分块（每块全新张量）；grad_a 拷回行切片，
    grad_b = sum(a_i^T @ grad_c_i) 用 torch 累加。"""
    m = a.shape[0]
    if m <= MATMUL_CHUNK_ROWS:
        bwd_kernel(a.contiguous(), b.contiguous(),
                   grad_c.contiguous(), grad_a, grad_b)
        return
    grad_b.zero_()
    b_c = b.contiguous()
    k_dim = a.shape[1]
    for s in range(0, m, MATMUL_CHUNK_ROWS):
        e = min(s + MATMUL_CHUNK_ROWS, m)
        a_c = a[s:e].clone()
        g_c = grad_c[s:e].clone()
        gx_c = torch.empty(e - s, k_dim, device=a.device, dtype=a.dtype)
        gw_c = torch.empty_like(grad_b)
        bwd_kernel(a_c, b_c, g_c, gx_c, gw_c)
        grad_a[s:e].copy_(gx_c)
        grad_b.add_(gw_c)


def chunked_vec_apply(fn, same_m_ins, other_ins, out,
                      chunk_rows=MATMUL_CHUNK_ROWS):
    """按行分块调用逐元素/逐行 kernel，规避大 M 的 vec kernel 编译时间爆炸
    （实测 (32768,128) vec kernel 单文件 bisheng 编译 3h+，而 (8192,·) 仅
    数十秒）。same_m_ins 为与输出同 M 行的输入列表，other_ins 为广播等其他
    输入（如 bias 向量，原样传入）；fn(*same_m_slices, *other_ins, out_slice)。
    逐行/逐元素语义下分块与整体等价。"""
    m = same_m_ins[0].shape[0]
    if m <= chunk_rows:
        fn(*[t.contiguous() for t in same_m_ins],
           *[t.contiguous() for t in other_ins], out)
        return
    for s in range(0, m, chunk_rows):
        e = min(s + chunk_rows, m)
        fn(*[t[s:e].contiguous() for t in same_m_ins],
           *[t.contiguous() for t in other_ins], out[s:e])


def chunked_bias_add_bwd(grad_c, grad_b, bwd_kernel,
                         chunk_rows=MATMUL_CHUNK_ROWS):
    """bias 反向（grad_b = sum(grad_c, 0)）：按 M 分块后 torch 累加，
    避免大 M 的 vec 归约 kernel 编译爆炸。"""
    m = grad_c.shape[0]
    if m <= chunk_rows:
        bwd_kernel(grad_c.contiguous(), grad_c.contiguous(), grad_b)
        return
    grad_b.zero_()
    for s in range(0, m, chunk_rows):
        e = min(s + chunk_rows, m)
        gb = torch.empty_like(grad_b)
        bwd_kernel(grad_c[s:e].contiguous(), grad_c[s:e].contiguous(), gb)
        grad_b.add_(gb)
...
#    反而会触发该问题，融合路径更稳）。
#    反向复用 matmul_bwd_kernel + bias_add_bwd_kernel；大 M 时前反向均
#    按 MATMUL_CHUNK_ROWS 分块（见 chunked_matmul_fwd/chunked_matmul_bwd）。
# ---------------------------------------------------------------------------
...
def make_pypto_linear_fused(matmul_fwd_kernel, matmul_bwd_kernel,
                            bias_add_fwd_kernel, bias_bwd_kernel):
    """创建线性层算子（y = x @ W + b）。

    前向拆为 matmul + bias_add 两次 kernel：实测在大输出（N=20256，如 MLM
    头）场景下，单 kernel 融合 matmul+bias（linear_fwd_kernel）因 cube→vec
    流水冲突（L0C 满、无法 double buffer）耗时 28.4ms，而拆开后仅 4.0ms
    （torch 参考 4.6ms）。小输出场景两者差距不大，统一采用拆开实现。
    """
    ...
    class PyPTOLinearFusedImpl(torch.autograd.Function):
        ...
        def forward(ctx, x, weight, bias):
            ...
            chunked_matmul_fwd(x, weight, c, matmul_fwd_kernel)
            if bias is not None:
                tmp = torch.empty_like(c)
                chunked_vec_apply(bias_add_fwd_kernel, [c], [bias], tmp)
                c.copy_(tmp)
            return c
        ...
        def backward(ctx, grad_c):
            ...
            if need_x or need_w:
                ...
                chunked_matmul_bwd(x, weight, grad_c, tmp_x, tmp_w,
                                   matmul_bwd_kernel)
            grad_b = None
            if need_b:
                grad_b = torch.empty(weight.shape[1], device=x.device,
                                     dtype=x.dtype)
                chunked_bias_add_bwd(grad_c, grad_b, bias_bwd_kernel)
...
def make_pypto_matmul(fwd_kernel, bwd_kernel):
    ...
    class PyPTOMatmulImpl(torch.autograd.Function):
        ...
        def forward(ctx, a, b):
            ...
            chunked_matmul_fwd(a, b, c, fwd_kernel)
            return c
        ...
        def backward(ctx, grad_c):
            ...
            chunked_matmul_bwd(a, b, grad_c, tmp_a, tmp_b, bwd_kernel)
...
def make_pypto_bias_add(fwd_kernel, bwd_kernel):
    ...
    class PyPTOBiasAddImpl(torch.autograd.Function):
        ...
        def forward(ctx, a, b):
            ...
            chunked_vec_apply(fwd_kernel, [a], [b], c)
            return c
        ...
        def backward(ctx, grad_c):
            ...
            if need_grad_b:
                chunked_bias_add_bwd(grad_c, grad_b, bwd_kernel)
            return grad_a, grad_b
...
def make_pypto_tanh(fwd_kernel, bwd_kernel):
    ...
    class PyPTOTanhImpl(torch.autograd.Function):
        ...
        def forward(ctx, a):
            ...
            chunked_vec_apply(fwd_kernel, [a_2d], [], b_2d)
        ...
        def backward(ctx, grad_b):
            ...
            if ctx.needs_input_grad[0]:
                chunked_vec_apply(bwd_kernel, [b_2d, grad_b_2d], [],
                                  grad_a_2d)
...
def make_pypto_relu(fwd_kernel, bwd_kernel):
    ...
    class PyPTOReLUOpImpl(torch.autograd.Function):
        ...
        def forward(ctx, x):
            ...
            chunked_vec_apply(fwd_kernel, [x_2d], [], y_2d)
        ...
        def backward(ctx, grad_out):
            ...
            if ctx.needs_input_grad[0]:
                chunked_vec_apply(bwd_kernel, [y_2d, grad_out_2d], [],
                                  grad_in_2d)
...
def _vec_tile_rows_for_cols(cols):
    """根据列数选择 UB 安全的向量 tile 行数（softmax 内部 DIV 等约需
    2×rows×cols×4B，须 < 192KB；大 vocab（如 MLM V=10000/20256）下
    8 行会溢出，实测 (6,10000) 报 240000B > MEM_UB 196608）。"""
    if cols <= 160:
        return 128
    if cols <= 320:
        return 64
...
def softmax_fwd_kernel(
    ...
    rows = _vec_tile_rows_for_cols(cols)
...
class PyPTOSoftmaxCrossEntropyLossFunction(torch.autograd.Function):
    ...
    def forward(ctx, x, y, num_classes):
        ...
        ce_out = torch.empty(m, device=x.device, dtype=x.dtype)
        if m <= MATMUL_CHUNK_ROWS:
            softmax_fwd_kernel(x, softmax_out, num_classes)
            cross_entropy_fwd_kernel(softmax_out, y_i32, ce_out,
                                     num_classes)
        else:
            # softmax/CE 逐行独立，按行分块规避大 M 的 vec kernel 编译爆炸
            for s in range(0, m, MATMUL_CHUNK_ROWS):
                e = min(s + MATMUL_CHUNK_ROWS, m)
                softmax_fwd_kernel(x[s:e].contiguous(),
                                   softmax_out[s:e], num_classes)
                cross_entropy_fwd_kernel(softmax_out[s:e],
                                         y_i32[s:e], ce_out[s:e],
                                         num_classes)
...
PyPTOLinearFused = make_pypto_linear_fused(
    matmul_fwd_kernel, matmul_bwd_kernel, bias_add_fwd_kernel,
    bias_add_bwd_kernel)
...
class PyPTOLinear(nn.Module):
    ...
    def forward(self, x):
        ...
        # 2D 非连续视图（如 X[:, 0, :]）无法被 kernel 直接接受（from_torch 报
        # "not all tensors are contiguous"），这里统一显式拷贝（已连续时零开销）
        x = x.contiguous()
        orig_m = x.shape[0]
        padded = False
        if orig_m % 32 != 0:
        ...
        if self.bias is not None:
            # 统一走 PyPTOLinearFused（matmul + bias_add 拆开两次 kernel）：
            # 大输出（N=20256，MLM 头）下单 kernel 融合因 cube→vec 流水冲突
            # 慢 7 倍（实测 28.4ms vs 4.0ms），拆开后与 torch 相当且更稳。
...
# ---------------------------------------------------------------------------
# cosine_sim：余弦相似度 cos = W·x / (‖W‖·‖x‖) 融合 kernel（13.7 节 knn）。
#   W: (V, d) 词表×维度；x: (d, 1) 查询向量；W_norm_sq: (V, 1) 行范数平方
#   （加载时预计算一次，避免每次查询重算 O(V·d)）；x_norm_sq 是 d 维小量
#   的一次归约，由宿主侧算好后以标量传入。
#   kernel 内：dot = W @ x（cube），cos = dot · rsqrt(W_norm_sq·x_norm_sq + 1e-9)
#   （vec，tiling (128,8) 适配 (V,1) 列向量，实测 400001×50 与 torch 全一致）。
#   仅推理场景（检索 topk），不实现反向。
# ---------------------------------------------------------------------------
...
def make_pypto_cosine_sim(fwd_kernel):
    """创建 PyPTO 余弦相似度算子（13.7 节 knn 检索，仅推理）。

        forward 输入：w (V, d)、x (d, 1)、w_norm_sq (V, 1)（预计算行范数
        平方），返回 (V, 1) 余弦相似度；无梯度需求，反向直接报错。
    """
...
class PyPTOLayerNormModule(nn.Module):
    ...
    def forward(self, X):
        ...
        w1 = self.weight.reshape(-1)
        b1 = self.bias.reshape(-1)
        if x2.shape[0] <= MATMUL_CHUNK_ROWS:
            y2 = PyPTOLayerNorm.apply(x2, w1, b1)
        else:
            # LayerNorm 逐行独立，按行分块规避大 M 的 vec kernel 编译爆炸
            ys = []
            for s in range(0, x2.shape[0], MATMUL_CHUNK_ROWS):
                e = min(s + MATMUL_CHUNK_ROWS, x2.shape[0])
                ys.append(PyPTOLayerNorm.apply(x2[s:e], w1, b1))
            y2 = torch.cat(ys, 0)
...
# =========================================================================
# ch13 新增：带掩码的二元交叉熵损失（word2vec 负采样训练）
# 语义等价 torch.nn.functional.binary_cross_entropy_with_logits(
#     inputs, target, weight=mask, reduction='none').mean(dim=1)
#   out[n] = (1/L) * sum_l mask[n,l] * (-[y*log(sig(x)) + (1-y)*log(1-sig(x))])
# 反向：grad_x[n,l] = (1/L) * mask[n,l] * (sig(x[n,l]) - y[n,l])
# =========================================================================
