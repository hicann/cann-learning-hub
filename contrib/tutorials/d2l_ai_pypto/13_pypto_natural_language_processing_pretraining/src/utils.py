...
覆盖函数：
- 文本处理：read_time_machine, tokenize, Vocab, get_tokens_and_segments
- 数据迭代：seq_data_iter_random, seq_data_iter_sequential, SeqDataLoader
- 数据加载：get_dataloader_workers（load_data_ptb / load_data_wiki 复用）
...
- BERT 模型（13.8 节，PyPTO 算子版）：AddNorm, PositionWiseFFN, EncoderBlock,
  BERTEncoder, MaskLM, NextSentencePred, PyPTOTanhModule, BERTModel
"""
...
# ch13 数据集条目（word2vec 的 PTB 语料库、BERT 预训练的 WikiText-2）
DATA_HUB['ptb'] = (DATA_URL + 'ptb.zip',
                   '319d85e578af0cdc590547f26231e4e31cdf1e42')
# 注：上游 S3 源（s3.amazonaws.com/research.metamind.io）已失效（301/403）。
# 本条目已替换为从 HF 镜像（hf-mirror.com Salesforce/wikitext，tokenized 版
# parquet）重建的 wikitext-2-v1.zip（目录结构 wikitext-2/wiki.{train,valid,test}.tokens
# 与原版一致，train 36718 行）。zip 由 _download_wikitext2 在首次使用时重建，
# 此处 URL/sha1 仅为占位（见 _download_wikitext2）。
DATA_HUB['wikitext-2'] = (
    'https://hf-mirror.com/datasets/Salesforce/wikitext/resolve/main/',
    'placeholder-rebuilt-from-hf-parquet')
...
def download(name, cache_dir=os.path.join("..", "data")):
    ...
    if name == "wikitext-2":
        return _download_wikitext2(cache_dir)
...
def _download_wikitext2(cache_dir):
    """重建并缓存 wikitext-2-v1.zip。

    上游 S3 源已失效（301/403），从 HF 镜像（hf-mirror.com
    Salesforce/wikitext）下载 tokenized 版 parquet（每行一个段落，含
    <unk>/@-@ 等标记，与原版 wiki.train.tokens 一致），重建
    wikitext-2-v1.zip（目录结构 wikitext-2/wiki.{train,valid,test}.tokens）。
    parquet 的 text 为 HF tokenizer 输出（段落内换行转为空格），与原版
    "每行一段落、句子间用 ' . ' 连接" 的格式兼容，_read_wiki 可直接解析。
    """
    import pandas as pd
    import pyarrow.parquet as pq

    os.makedirs(cache_dir, exist_ok=True)
    fname = os.path.join(cache_dir, "wikitext-2-v1.zip")
    if os.path.exists(fname):
        try:
            with zipfile.ZipFile(fname) as z:
                if z.namelist() == ["wikitext-2/wiki.train.tokens",
                                    "wikitext-2/wiki.test.tokens",
                                    "wikitext-2/wiki.valid.tokens"]:
                    return fname
        except zipfile.BadZipFile:
            pass
    base = DATA_HUB["wikitext-2"][0]
    opener = urllib.request.build_opener()
    opener.addheaders = [("User-Agent", "Mozilla/5.0")]
    parts = {}
    print("正在从 HF 镜像下载 wikitext-2 parquet 并重建 zip...")
    for split, out_name in (("train", "wiki.train.tokens"),
                            ("validation", "wiki.valid.tokens"),
                            ("test", "wiki.test.tokens")):
        url = (base + f"wikitext-2-v1/{split}-00000-of-00001.parquet")
        pq_path = os.path.join(cache_dir, f"wt2_{split}.parquet")
    ...
    with zipfile.ZipFile(fname, "w") as z:
        for out_name, text in parts.items():
            z.writestr(f"wikitext-2/{out_name}", text)
    print(f"wikitext-2 已重建：{fname}（train {len(parts['wiki.train.tokens'].splitlines())} 行）")
    return fname
...
def get_tokens_and_segments(tokens_a, tokens_b=None):
    """获取输入序列的词元及其片段索引（BERT 输入表示）。

    首次出现在 13.8 节（notebook 内联完整实现），此处收录供 13.9 节及
    后续章节复用（与 d2l 的 ``#@save`` 模式一致）。
    """
...
# =========================================================================
# ch13 新增：word2vec 预训练数据管线（PTB 语料库）
# 分步讲解见 13.3 节 notebook；此处收录完整实现，供 13.4 节直接复用
# =========================================================================
...
def load_data_ptb(batch_size, max_window_size, num_noise_words):
    """下载PTB数据集，然后将其加载到内存中"""
    num_workers = 0  # 笔记本环境使用主进程加载数据（d2l 原书为多进程加载）
    sentences = read_ptb()
...
# =========================================================================
# ch13 新增：BERT 预训练数据管线（WikiText-2 语料库）
# 分步讲解见 13.9 节 notebook；此处收录完整实现，供 13.10 节直接复用
# =========================================================================
...
# =========================================================================
# ch13 新增：BERT 模型（PyPTO 算子版）
# 分步讲解与演示见 13.8 节 notebook；此处收录完整实现，供 13.10 节直接复用
# （等价于原书 13.8 节 #@save 后 13.10 节直接使用 d2l.BERTModel）
# 依赖 src.pypto_ops / src.attention，为避免循环导入在方法内局部导入
# =========================================================================


class AddNorm(nn.Module):
    """残差连接后进行层规范化（PyPTO 算子版）"""

    def __init__(self, normalized_shape, dropout, **kwargs):
        from src.pypto_ops import PyPTOLayerNormModule
...
class PositionWiseFFN(nn.Module):
    ...
    def __init__(self, ffn_num_input, ffn_num_hiddens, ffn_num_outputs,
                 **kwargs):
        from src.pypto_ops import PyPTOLinear, PyPTOReLU
...
class EncoderBlock(nn.Module):
    """Transformer编码器块（PyPTO 算子版，对应原书 d2l.EncoderBlock）"""
...
class MaskLM(nn.Module):
    """BERT的掩蔽语言模型任务（PyPTO 算子版）"""

    def __init__(self, vocab_size, num_hiddens, num_inputs=768, **kwargs):
        from src.pypto_ops import PyPTOLinear, PyPTOReLU, PyPTOLayerNormModule
        super(MaskLM, self).__init__(**kwargs)
        self.mlp = nn.Sequential(PyPTOLinear(num_inputs, num_hiddens),
                                 PyPTOReLU(),
                                 PyPTOLayerNormModule(num_hiddens),
                                 PyPTOLinear(num_hiddens, vocab_size))
...
class NextSentencePred(nn.Module):
    """BERT的下一句预测任务（PyPTO 算子版）"""

    def __init__(self, num_inputs, **kwargs):
        from src.pypto_ops import PyPTOLinear
        super(NextSentencePred, self).__init__(**kwargs)
        self.output = PyPTOLinear(num_inputs, 2)
...
class PyPTOTanhModule(nn.Module):
    """PyPTO tanh 激活封装（13.8 节定义，此处收录供 BERTModel 复用）"""

    def forward(self, X):
        from src.pypto_ops import PyPTOTanh
        return PyPTOTanh.apply(X)


class BERTModel(nn.Module):
    """BERT模型（PyPTO 算子版，结构同 13.8 节）"""
    ...
    def __init__(self, vocab_size, num_hiddens, norm_shape, ffn_num_input,
        ...
        self.hidden = nn.Sequential(PyPTOLinear(hid_in_features, num_hiddens),
                                    PyPTOTanhModule())
