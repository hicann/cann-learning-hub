...
namespace lslidar_driver
{
	...
	LslidarDriver::LslidarDriver(rclcpp::Node* node) : node_(node)
	{
		return;
	}
	...
	void LslidarDriver::initParam() // 初始化驱动参数
	{
		...
		difop_switch = node_->create_subscription<std_msgs::msg::Int8>(
			"lslidar_order", 10, std::bind(&LslidarDriver::lidar_order, this, std::placeholders::_1)); // 指令输入
		is_start = true;
		node_->declare_parameter<std::string>("lidar_name", "M10");
		node_->declare_parameter<std::string>("frame_id", "laser_link");
		node_->declare_parameter<std::string>("scan_topic", "/scan");
		node_->declare_parameter<std::string>("interface_selection", "net");
		node_->declare_parameter<std::string>("pointcloud_topic", "lslidar_point_cloud");
		node_->declare_parameter<bool>("pubScan", true);
		node_->declare_parameter<bool>("use_gps_ts", false);
		node_->declare_parameter<bool>("compensation", false);
		node_->declare_parameter<bool>("pubPointCloud2", false);
		node_->declare_parameter<bool>("high_reflection", false);
		node_->declare_parameter<double>("min_range", 0.3);
		node_->declare_parameter<double>("max_range", 100.0);
		node_->declare_parameter<double>("angle_disable_min", 0.0);
		node_->declare_parameter<double>("angle_disable_max", 0.0);
		node_->declare_parameter<std::string>("serial_port", "/dev/ttyUSB0");
		node_->get_parameter("lidar_name", lidar_name);
		node_->get_parameter("frame_id", frame_id);
		node_->get_parameter("scan_topic", scan_topic);
		node_->get_parameter("interface_selection", interface_selection);
		node_->get_parameter("pointcloud_topic", pointcloud_topic);
		node_->get_parameter("pubScan", pubScan);
		node_->get_parameter("use_gps_ts", use_gps_ts);
		node_->get_parameter("compensation", compensation);
		node_->get_parameter("pubPointCloud2", pubPointCloud2);
		node_->get_parameter("high_reflection", high_reflection);
		...
		RCLCPP_INFO_STREAM(node_->get_logger(), "Lidar is " << lidar_name);
		return;
	}
	...
	void LslidarDriver::lidar_order(const std_msgs::msg::Int8::SharedPtr msg) // 发送控制雷达指令
	{
		if (lidar_name == "L10")
			return;
		int i = msg->data;
		...
		else
		{
			...
			for (int k = 0; k < 10; k++)
			{
				...
				{
					RCLCPP_INFO(node_->get_logger(), "Successfully set!");
					if (i == 1)
						usleep(1000000); // 1.0s
					if (i == 0)
						is_start = false;
	...
	void LslidarDriver::open_serial() // 打开串口
	{
		...
		node_->get_parameter_or("serial_port", serial_port_, default_port);
		serial_ = LSIOSR::instance(serial_port_, baud_rate_);
		code = serial_->init();
		if (code != 0)
		{
			RCLCPP_WARN(node_->get_logger(), "open_port %s ERROR !", serial_port_.c_str());
			rclcpp::shutdown();
			exit(0);
		}
	...
	{
		node_->declare_parameter<int>("device_port", 2368);
		node_->get_parameter("device_port", UDP_PORT_NUMBER);
		RCLCPP_INFO_STREAM(node_->get_logger(), "Opening UDP socket: port " << UDP_PORT_NUMBER);
		// ROS diagnostics
		diagnostics = std::make_shared<diagnostic_updater::Updater>(node_);
		diagnostics->setHardwareID("Lslidar");

		const double diag_freq = 12 * 24;
		diag_max_freq = diag_freq;
		diag_min_freq = diag_freq;
		RCLCPP_INFO(node_->get_logger(), "expected frequency: %.3f (Hz)", diag_freq);
		...
		if (dump_file != "")
		{
			msop_input_.reset(new lslidar_driver::InputPCAP(node_, UDP_PORT_NUMBER, packet_rate, dump_file));
		}
		else
		{
			msop_input_.reset(new lslidar_driver::InputSocket(node_, UDP_PORT_NUMBER));
		}

		return true;
	}

	int LslidarDriver::getScan(std::vector<ScanPoint> &points, rclcpp::Time &scan_time, double &scan_duration) // 获取数据和相对时间
	{
	...
	{
		initParam();
		// 用 node_->now() 初始化，避免 getScan() 中 time_ - pre_time_ 因时间源不同抛错
		pre_time_ = node_->now();
		time_ = node_->now();
		if (interface_selection == "net")
		{
			if (!createRosIO())
			{
				RCLCPP_ERROR(node_->get_logger(), "Cannot create all ROS IO...");
				return false;
			}
		}
		else
		{
			node_->declare_parameter<std::string>("in_file_name", "");
			node_->get_parameter("in_file_name", in_file_name);
			RCLCPP_INFO_STREAM(node_->get_logger(), "Opening PCAP file " << in_file_name);
			if (in_file_name == "")
				open_serial();
			else
			{
				...
				{
					RCLCPP_ERROR(node_->get_logger(), "Cannot open the file");
					return false;
				}
			}
		}
		...
		if (pubScan)
			pub_ = node_->create_publisher<sensor_msgs::msg::LaserScan>(scan_topic, 3);
		if (pubPointCloud2)
			pointcloud_pub = node_->create_publisher<sensor_msgs::msg::PointCloud2>(pointcloud_topic, 10);
		RCLCPP_INFO(node_->get_logger(), "Initialised lslidar without error");
		return true;
	}
	...
	void LslidarDriver::recvThread_crc(int &count, int &link_time) // 获取串口数据时检测是否超时
	{
		...
		if (link_time > 150)
		{
			RCLCPP_ERROR(node_->get_logger(), "long time lose data!\n"); 
			serial_->close();
			int ret = serial_->init();
			if (ret < 0)
			{
				RCLCPP_WARN(node_->get_logger(), "serial open fail");
				usleep(1000000);
			}
			link_time = 0;
		}
	...
	void LslidarDriver::data_processing(unsigned char *packet_bytes, int len) // 处理每一包的数据
	{
		...
		if (invalidValue <= 1)
		{
			// delete[] packet_bytes;
			// packet_bytes = NULL;
			RCLCPP_WARN(node_->get_logger(), "Valid point is %d", invalidValue);
			return;
		}
	...
	{
		RCLCPP_INFO(node_->get_logger(), "pubscanthread");
		...
		while (rclcpp::ok())
		{
			...
			if (lidar_name == "N10_P" || lidar_name == "M10_DOUBLE")
			{
				if (pubScan)
				{
					...
					if (use_gps_ts)
					{
						msg.header.stamp = rclcpp::Time(static_cast<int32_t>(sweep_end_time_gps), static_cast<uint32_t>(sweep_end_time_hardware));
					}
					else
					{
				...
				if (pubPointCloud2)
				{
					...
					if (use_gps_ts)
					{
						start_time = rclcpp::Time(static_cast<int32_t>(sweep_end_time_gps), static_cast<uint32_t>(sweep_end_time_hardware));
					}
	...
	bool LslidarDriver::polling() // 主循环函数
	{
		...
		if (interface_selection == "net")
		{
			...
			lslidar_msgs::msg::LslidarPacket::SharedPtr packet = std::make_shared<lslidar_msgs::msg::LslidarPacket>();
			struct timeval tv;
