...
/*
 * 实验5.1 msSanitizer 标准算子工程版：Add 算子 Host 侧（Tiling / InferShape / OpDef）。
 * 课程适配：以模板 add_custom_template.cpp 为基础，仅重命名算子类型与 Tiling 结构，
 * Tiling 逻辑与 InferShape/InferDataType 原样保留。
 */

#include "register/op_def_registry.h"
#include "../op_kernel/add_custom_msanitizer_tiling.h"
...
static ge::graphStatus TilingFunc(gert::TilingContext *context)
{
    ...
    TilingDataMsanitizer *tiling = context->GetTilingData<TilingDataMsanitizer>();
...
namespace ops {
class AddCustomMsanitizer : public OpDef {
public:
    explicit AddCustomMsanitizer(const char *name) : OpDef(name)
    {
        this->Input("x")
            .ParamType(REQUIRED)
