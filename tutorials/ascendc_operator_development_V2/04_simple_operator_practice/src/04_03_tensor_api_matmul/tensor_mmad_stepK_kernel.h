    ...
    for (uint32_t mi = 0; mi < mLoop; ++mi) {
        for (uint32_t ni = 0; ni < nLoop; ++ni) {
            for (uint32_t ki = 0; ki < kLoop; ++ki) {
                ...
                //   A: gmASingle.slice(make_coord(mi * baseM, ki * baseK), make_shape(baseM, stepK * baseK))
                //   B: gmBSingle.slice(make_coord(ki * baseK, ni * baseN), make_shape(stepK * baseK, baseN))
                if (ki % stepK == 0) {
                ...
                //   A: l1ATensor.slice(make_coord(0, (ki % stepK) * baseK), make_shape(baseM, baseK))
                //   B: l1BTensor.slice(make_coord((ki % stepK) * baseK, 0), make_shape(baseK, baseN))
