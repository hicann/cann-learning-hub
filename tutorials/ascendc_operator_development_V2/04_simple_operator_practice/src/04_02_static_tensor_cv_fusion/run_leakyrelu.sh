...
for arg in "$@"; do
  case "${arg}" in
    ...
    -h|--help)
      echo "用法: bash run_leakyrelu.sh [--mode=sim|npu] [--npu-arch=dav-2201|dav-3510] [--compile-only] [--force-run]"
      exit 0
      ;;
    *)
      echo "未知参数: ${arg}"
...
cmake --build build --target leakyrelu_demo -j"$(nproc)"

if [ "${COMPILE_ONLY}" -eq 1 ]; then
  echo "compile pass: leakyrelu_demo"
  exit 0
fi
...
python3 scripts/gen_data.py --sample=leakyrelu
timeout "${SAMPLE_RUN_TIMEOUT:-120}" ./build/leakyrelu_demo
