...
for arg in "$@"; do
  case "${arg}" in
    ...
    -h|--help)
      echo "用法: bash run_mmad.sh [--mode=sim|npu] [--npu-arch=dav-2201|dav-3510] [--compile-only] [--force-run]"
      exit 0
      ;;
    *)
      echo "未知参数: ${arg}"
...
cmake --build build --target mmad_basic_demo -j"$(nproc)"

if [ "${COMPILE_ONLY}" -eq 1 ]; then
  echo "compile pass: mmad_basic_demo"
  exit 0
fi
...
python3 scripts/gen_data.py --sample=mmad
timeout "${SAMPLE_RUN_TIMEOUT:-120}" ./build/mmad_basic_demo
