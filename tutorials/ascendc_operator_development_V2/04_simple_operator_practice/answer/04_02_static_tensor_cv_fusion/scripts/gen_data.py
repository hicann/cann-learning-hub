...
def gen_golden_data(sample):
    ...
    if sample == "mmad_leakyrelu":
        residual_gm = (np.random.randint(-8, 9, [m, n]) / 64).astype(np.float16)
        golden = golden.astype(np.float16)
        golden = golden + residual_gm.astype(np.float16)
        residual_gm.tofile("./input/residual_gm.bin")
    golden = golden.astype(np.float16)
