...
constexpr uint32_t SINGLE_VF_DATA_LEN = WIDTH;
...
__global__ __vector__ void add_custom(__gm__ uint8_t* src0, __gm__ uint8_t* src1, __gm__ uint8_t* dst)
{
    ...
    // 申请UB内存
    __ubuf__ float src0_ub[128];
    __ubuf__ float src1_ub[128];
    __ubuf__ float dst_ub[128];

    __gm__ float* src0_gm = reinterpret_cast<__gm__ float*>(src0) + get_block_idx() * SINGLE_VF_DATA_LEN * LOOP_COUNT;
    __gm__ float* src1_gm = reinterpret_cast<__gm__ float*>(src1) + get_block_idx() * SINGLE_VF_DATA_LEN * LOOP_COUNT;
    __gm__ float* dst_gm = reinterpret_cast<__gm__ float*>(dst) + get_block_idx() * SINGLE_VF_DATA_LEN * LOOP_COUNT;
    ...
    for (uint32_t i = 0; i < LOOP_COUNT; i++) {
        ...
        asc_copy_gm2ub_align(src0_ub, &src0_gm[i * SINGLE_VF_DATA_LEN], 1, static_cast<uint16_t>(WIDTH * sizeof(float)), 0, 0, true, 0, 0, 0);
        asc_copy_gm2ub_align(src1_ub, &src1_gm[i * SINGLE_VF_DATA_LEN], 1, static_cast<uint16_t>(WIDTH * sizeof(float)), 0, 0, true, 0, 0, 0);
        ...
        asc_copy_ub2gm_align(&dst_gm[i * SINGLE_VF_DATA_LEN], dst_ub, 1, static_cast<uint16_t>(WIDTH * sizeof(float)), 0, 0, 0);
