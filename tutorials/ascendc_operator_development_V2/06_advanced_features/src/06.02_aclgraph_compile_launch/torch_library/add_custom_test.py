...
torch.ops.load_library(str(Path.cwd() / "libascendc_ops.so"))
...
def main():
    ...
    with torch.no_grad():
        torch.ops.ascendc_ops.ascendc_add(static_x, static_y)
    torch_npu.npu.synchronize()

    graph = torch_npu.npu.NPUGraph()
    with torch.no_grad(), torch_npu.npu.graph(graph):
        graph_output = torch.ops.ascendc_ops.ascendc_add(static_x, static_y)
    ...
    print("Torch Library custom operator Aclgraph replay success.")
