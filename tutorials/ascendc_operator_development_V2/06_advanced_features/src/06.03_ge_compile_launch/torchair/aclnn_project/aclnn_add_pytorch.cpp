...
at::Tensor AclnnAddMeta(const at::Tensor &x, const at::Tensor &y)
{
    TORCH_CHECK(x.sizes() == y.sizes(), "x and y must have the same shape.");
    TORCH_CHECK(x.dim() == 2 && x.size(0) == ROW_NUM && x.size(1) == COL_NUM,
        "aclnn_add only supports shape [8, 2048] in this sample.");
    TORCH_CHECK(x.scalar_type() == at::ScalarType::Float && y.scalar_type() == at::ScalarType::Float,
        "aclnn_add only supports float tensors in this sample.");
...
{
    m.def("aclnn_add(Tensor x, Tensor y) -> Tensor");
}

TORCH_LIBRARY_IMPL(ge_launch_samples, Meta, m)
{
    m.impl("aclnn_add", TORCH_FN(AclnnAddMeta));
}
