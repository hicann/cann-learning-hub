...
int main(int argc, char **argv)
{
    ...
    // 2. Create input and output, shape修改为 (7, 83)
    std::vector<int64_t> inputXShape = {999, 999};
    std::vector<int64_t> outputYShape = {999, 999};
    ...
    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnSigmoidCustomGetWorkspaceSize failed. ERROR: %d\n", ret);
    ...
    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnSigmoidCustom failed. ERROR: %d\n", ret);
              DestroyResources(tensors, deviceAddrs, stream, deviceId, workspaceAddr); return FAILED);
    ...
    // print the output result —— 完全还原你原始代码
    std::vector<float> goldenData(size, 0.7310585786300049f);
