...
USE_CORE_NUM = 8
TILE_NUM = 8
# 本文件为第六章章节实践参考答案：修复asc.mul误用+TQue双缓冲优化版
...
def compute(z_gm: asc.GlobalTensor, in_queue_x: asc.TQue, in_queue_y: asc.TQue, out_queue_z: asc.TQue,
    ...
    asc.mul(z_local, x_local, y_local, tile_length)
...
def vmul_launch(x: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
    ...
    vmul_kernel[USE_CORE_NUM, rt.current_stream()](x, y, z, block_length, tile_length)
    return z
