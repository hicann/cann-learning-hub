...
这些文件是通过 `torchrun` 直接执行的独立通信基准程序，不是供 notebook
import 的课程模块；输出目录和 profiler trace 也由命令行参数显式指定。
...
The commands intentionally do not set `ASCEND_RT_VISIBLE_DEVICES`. The scheduler must
