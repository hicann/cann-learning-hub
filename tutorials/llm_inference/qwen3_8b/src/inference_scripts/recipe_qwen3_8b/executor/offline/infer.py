...
def get_batch_parallel_info(config, global_rank):
    parallel_config = config.parallel_config
    if config.model_config.custom_params.get("enable_afd", False):
        ffn_world_size = parallel_config.world_size // 2
        role_rank = global_rank % ffn_world_size
        cache_parallel_size = (
            parallel_config.get_cache_parallel_size()
            if parallel_config.uses_kvp_cache_parallel()
            else 1
        )
        rank_group_size = parallel_config.attn_tp_size * cache_parallel_size
        batch_parallel_size = ffn_world_size // rank_group_size
        return batch_parallel_size, rank_group_size, role_rank

    if parallel_config.uses_kvp_cache_parallel():
        cache_parallel_size = parallel_config.get_cache_parallel_size()
        rank_group_size = parallel_config.attn_tp_size * cache_parallel_size
    else:
        rank_group_size = parallel_config.attn_tp_size
    return parallel_config.get_batch_parallel_size(), rank_group_size, global_rank
...
def main():
    ...
    batch_parallel_size, rank_group_size, role_rank = get_batch_parallel_info(config, global_rank)
    batch_size = config.scheduler_config.batch_size

    if batch_parallel_size > 1:
        if batch_size % batch_parallel_size != 0:
            raise ValueError(
                f"batch_size ({batch_size}) must be divisible by batch_parallel_size ({batch_parallel_size})"
            )
        batch_size_per_rank = batch_size // batch_parallel_size
        global_dp_rank = role_rank // rank_group_size
