...
class ParallelConfig:
    ...
    Attributes:
        ...
        kvp_size: KV cache sequence parallelism size (default: 1)
    """
    # Basic info
    world_size: int = 1
    global_rank: int = 0
    ...
    def uses_kvp_cache_parallel(self) -> bool:
        return self.kvp_size > 1 and self.attn_tp_size == 1 and self.cp_size == 1

    def get_cache_parallel_size(self) -> int:
        return self.kvp_size if self.uses_kvp_cache_parallel() else 1

    def get_batch_parallel_size(self) -> int:
        if self.uses_kvp_cache_parallel():
            return self.world_size // (self.attn_tp_size * self.kvp_size)
        return self.attn_dp_size
...
class SchedulerConfig:
    ...
    Attributes:
        ...
        decode_only: TPOT-only benchmark switch (default False). When True the
            scheduler skips real prefill and runs decode against a KV cache
            faked to ``input_truncated_len`` (uninitialized content); generated
            text is meaningless. Used to measure decode/TPOT latency at long
            context without paying the prefill cost.
    """
    batch_size: int = 1
    ...
    def from_dict(cls, scheduler_config_dict: dict) -> "SchedulerConfig":
        ...
        return cls(
            ...
            decode_only=scheduler_config_dict.get("decode_only", False),
            enable_thinking=scheduler_config_dict.get("enable_thinking", False),
        )


@dataclass
class DisaggConfig:
    """Configuration placeholder for offline inference."""
...
class InferenceConfig:
    ...
    def from_dict(
        ...
        batch_parallel_size = infer_config.parallel_config.get_batch_parallel_size()
        require_even_batch_split = infer_config.parallel_config.uses_kvp_cache_parallel()
        if infer_config.model_config.custom_params.get("enable_afd", False):
            batch_parallel_size = infer_config._update_afd_parallel_config()
            require_even_batch_split = True
        if require_even_batch_split and infer_config.scheduler_config.batch_size % batch_parallel_size != 0:
            raise ValueError(
                f"batch_size={infer_config.scheduler_config.batch_size} must be divisible by "
                f"batch_parallel_size={batch_parallel_size}"
            )
        infer_config.scheduler_config.batch_size_per_dp_rank = \
            infer_config.scheduler_config.batch_size // batch_parallel_size
    ...
    def _update_afd_parallel_config(self):
        ...
        cache_parallel_size = parallel_config.get_cache_parallel_size()
        return afd_world_size // (parallel_config.attn_tp_size * cache_parallel_size)
