...
class KVCacheManager:
    ...
    def __init__(
        ...
        self.kvp_rank = kvp_rank
        self.kvp_seq_partition = kvp_seq_partition

    def _get_local_num_tokens(self, num_tokens: int) -> int:
        if not self.kvp_seq_partition or self.kvp_size <= 1:
            return num_tokens
        return num_tokens // self.kvp_size + int(num_tokens % self.kvp_size > self.kvp_rank)
    ...
    def allocate_slots(
        ...
        total_computed_tokens = self._get_local_num_tokens(total_computed_tokens)
        num_tokens_need_slot = self._get_local_num_tokens(num_tokens_need_slot)
