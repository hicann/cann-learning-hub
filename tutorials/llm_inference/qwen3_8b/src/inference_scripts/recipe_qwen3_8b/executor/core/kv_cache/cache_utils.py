...
def get_kvp_local_num_tokens(num_tokens: int, kvp_size: int = 1, kvp_rank: int = 0) -> int:
    """Return local token count for one KVP sequence-partition rank."""
    if kvp_size <= 1:
        return num_tokens
    return num_tokens // kvp_size + int(num_tokens % kvp_size > kvp_rank)
...
def calculate_block_num(
    ...
    if offline_max_len:
        if kvp_seq_partition:
            offline_max_len = get_kvp_local_num_tokens(offline_max_len, kvp_size, kvp_rank)
    ...
    if kvp_seq_partition:
        required_tokens = get_kvp_local_num_tokens(required_tokens, kvp_size, kvp_rank)
...
def prepare_slot_mapping(
    ...
    for attn_type, block_table in block_tables.items():
        ...
        for idx in range(actual_seq_lengths_cu_q.shape[0]):
            ...
            if kvp_seq_partition and kvp_size > 1:
                active_mask = tmp_position_ids % kvp_size == kvp_rank
                temp_slot_mapping = tmp_position_ids.new_full(tmp_position_ids.shape, -1)
                local_position_ids = tmp_position_ids[active_mask] // kvp_size
                if local_position_ids.numel() > 0:
                    block_indices = local_position_ids // block_size
                    position_offsets = local_position_ids % block_size
                    max_block_index = int(block_indices.max().item())
                    ...
                    temp_slot_mapping[active_mask] = block_ids * block_size + position_offsets
            else:
