...
class QwenRMSNorm(nn.Module):
    def __init__(self, hidden_size, eps=1e-6, enable_npu_rmsnorm=True, enable_add_rmsnorm=True):
        super().__init__()
        self.weight = nn.Parameter(torch.ones(hidden_size))
        self.variance_epsilon = eps
        self.enable_npu_rmsnorm = enable_npu_rmsnorm
        self.enable_add_rmsnorm = enable_add_rmsnorm

    def ln(self, hidden_states):
        if self.enable_npu_rmsnorm:
...
class QwenAttention(nn.Module):
    ...
    def __init__(
        ...
        if self.use_qk_norm:
            enable_npu_rmsnorm = infer_config.model_config.custom_params.get("enable_npu_rmsnorm", True)
            self.q_norm = QwenRMSNorm(
                self.head_dim, eps=config.rms_norm_eps, enable_npu_rmsnorm=enable_npu_rmsnorm
            )
...
class QwenDecoderLayer(nn.Module):
    def __init__(
        ...
        enable_npu_rmsnorm = infer_config.model_config.custom_params.get("enable_npu_rmsnorm", True)
        enable_add_rmsnorm = infer_config.model_config.custom_params.get("enable_add_rmsnorm", True)
        self.input_layernorm = QwenRMSNorm(
            config.hidden_size,
            eps=config.rms_norm_eps,
            enable_npu_rmsnorm=enable_npu_rmsnorm,
            enable_add_rmsnorm=enable_add_rmsnorm,
        )
