"""Helpers for the Qwen3-1.7B recipes-style notebook workflow."""
...
from .inference_qwen3_1_7b import (
    build_output_rows,
    count_prompt_tokens,
...
def find_recipe_case_dir(recipe_root: Path, yaml_path: Path) -> Path | None:
    config = yaml.safe_load(yaml_path.read_text(encoding="utf-8"))
    model_name = config.get("model_config", {}).get("model_name", "qwen3_1_7b")
