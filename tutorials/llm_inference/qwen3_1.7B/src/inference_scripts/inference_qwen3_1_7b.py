#!/usr/bin/env python3
"""Shared parsing and metrics helpers for the Qwen3-1.7B recipes notebooks."""
...
os.environ.setdefault("QWEN3_1_7B_DOWNLOAD_BACKEND", "modelscope")
...
RECIPE_ROOT = Path(__file__).resolve().parent / "recipe_qwen3_1_7b"
...
def resolve_model_path(model_path: str, quiet_model_io: bool = False) -> tuple[str, str]:
    ...
    backend = os.environ.get("QWEN3_1_7B_DOWNLOAD_BACKEND", "modelscope").strip().lower()
    if backend in {"modelscope", "ms"}:
        ...
        try:
            ...
            raise RuntimeError(
                ...
                "QWEN3_1_7B_MODEL_PATH 指向本地模型目录，或设置 "
                "QWEN3_1_7B_DOWNLOAD_BACKEND=huggingface。"
            ) from exc
    ...
    raise ValueError("QWEN3_1_7B_DOWNLOAD_BACKEND 只能是 'modelscope' 或 'huggingface'。")
...
def find_recipe_case_log(yaml_path: Path) -> Path | None:
    ...
    expected = RECIPE_ROOT / "models" / "qwen" / "res" / date / f"qwen3_1_7b_{yaml_path.stem}" / "log_0.log"
    if expected.is_file():
        return expected
