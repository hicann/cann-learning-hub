...
class Scheduler:
    ...
    def _schedule_batch(
        ...
        if self.config.decode_only and self.waiting_queue:
            self._admit_decode_only_requests(engine)
    ...
    def _admit_decode_only_requests(self, engine: ExecutionEngine) -> None:
        """Admit waiting requests straight into decode for TPOT benchmarking.

        ``decode_only`` skips real prefill: each request's KV cache is faked to
        ``input_truncated_len`` positions (uninitialized content) and the request
        is moved directly into the decode running set, mirroring the legacy
        ``decode_only`` runner path. This measures decode/TPOT latency at long
        context without paying the prefill cost; generated text is meaningless.
        Only reached when ``self.config.decode_only`` is True.
        """
        if engine.next_n > 0:
            raise ValueError("decode_only does not support MTP; set next_n to 0 or disable decode_only.")

        while self.waiting_queue:
            request = self.waiting_queue.popleft()
            self._prepare_request_prompt(request)
            if engine.kvcache_manager and not engine.kvcache_manager.allocate_slots(
                    request.request_id, computed_tokens=0, num_new_tokens=self.input_truncated_len):
                raise RuntimeError(
                    f"decode_only: failed to allocate {self.input_truncated_len} KV slots for "
                    f"request {request.request_id}; reduce input_truncated_len or batch_size."
                )
            request.computed_len = self.input_truncated_len - 1
