...
class ModelCacheInfo:
    ...
    # True for models that shard sequence positions across KVP ranks when writing
    # paged KV cache. The default keeps existing models on the full-sequence path.
    ...
    def merge(self, other: "ModelCacheInfo") -> "ModelCacheInfo":
        ...
        if self.kvp_seq_partition != other.kvp_seq_partition:
            raise ValueError(
                "kvp_seq_partition mismatch across merged cache infos: "
                f"{self.kvp_seq_partition} vs {other.kvp_seq_partition}"
            )
